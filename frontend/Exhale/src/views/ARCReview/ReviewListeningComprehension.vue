...
const updateProblemDetails = () => {
    ...
    problem.question.value = problemSet[problemIdx].question;
    problem.options.value = problemSet[problemIdx].options;
    problem.questionImage.value = problemSet[problemIdx].questionImage;
    problem.categoryId.value = problemSet[problemIdx].category_id;
    problem.categoryName.value = problemSet[problemIdx].category_name;
      ...
      <div class="content">
        <div>
          ...
          <img src="@/assets/triangle_left.svg" class="triangle_left">
            ...
            <div v-if="problem.categoryId.value==6" class="options-grid">
                <div v-for="(img, idx) of problem.options.value" :key="idx" class="option-item" @click="selectOption(idx)"> 
                        <img :src="img" alt="option" class="option-image">
                </div>
            </div>
            <div v-if="problem.categoryId.value==7" class="options-grid2">
                  <img :src="problem.questionImage.value" class="option-image2"/>
                  <div v-for="(option, idx) of problem.options.value" :key="idx"> 
                    <label class="option2" @click="selectOption(idx)">
                      &nbsp;{{ idx+1 }}. &nbsp;&nbsp;&nbsp;&nbsp; {{ option}}
                      </label>
                  </div>
                </div>
...
.options-grid {
  display: grid;
  grid-template-columns: repeat(2, 1fr); // 2개의 컬럼으로 표시
  grid-gap: 20px 20px; 
